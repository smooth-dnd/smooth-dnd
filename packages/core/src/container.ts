...
function isDragRelevant({ element, getOptions }: ContainerProps) {
  return function (sourceContainer: IContainer, payload: any) {
    ...
    if (options.behavior === 'copy') return false
...
function findDraggableAtPos({ layout }: { layout: LayoutManager }) {
  const find = (draggables: HTMLElement[], pos: number, startIndex: number, endIndex: number, withRespectToMiddlePoints = false): number | null => {
    if (endIndex < startIndex) {
      return startIndex
    }
    // binary search draggable
...
function getContainerProps(element: HTMLElement, getOptions: () => ContainerOptions): ContainerProps {
  ...
  const options = getOptions()
  // set flex classes before layout is init for scroll listener
...
function getRemovedItem({ element, getOptions }: ContainerProps) {
  ...
  return ({ draggableInfo }: DragInfo) => {
    ...
    if (prevRemovedIndex == null && draggableInfo.container.element === element && getOptions().behavior !== 'copy') {
...
function setRemovedItemVisibility({ draggables, layout }: ContainerProps) {
...
function getPosition({ element, layout }: ContainerProps) {
  return ({ draggableInfo }: DragInfo) => {
    ...
    // TODO: if center is out of bounds use mouse position for hit-test
    // if (!hitElement) {
...
function getDragInsertionIndex({ draggables, layout }: ContainerProps) {
  const findDraggable = findDraggableAtPos({ layout })
...
function handleInsertionSizeChange({ element, draggables, layout, getOptions }: ContainerProps) {
  let stretcherElement: HTMLElement | null = null
      ...
      if (addedIndex !== null) {
        if (!stretcherElement) {
          ...
          if (lastDraggableEnd + elementSize > containerEnd) {
            stretcherElement = window.document.createElement('div') as HTMLElement
            stretcherElement.className = stretcherElementClass + ' ' + getOptions().orientation
            const stretcherSize = draggables.length > 0 ? elementSize + lastDraggableEnd - containerEnd : elementSize
            layout.setSize(stretcherElement.style, `${stretcherSize}px`)
            element.appendChild(stretcherElement)
            element[stretcherElementInstance] = stretcherElement
            return {
        ...
        if (stretcherElement) {
          layout.setTranslation(stretcherElement, 0)
          let toRemove = stretcherElement
          stretcherElement = null
...
function Container(element: HTMLElement): (options?: ContainerOptions) => IContainer {
  return function (options?: ContainerOptions): IContainer {
    ...
    const container: IContainer = {
      ...
      getTranslateCalculator(result: { dragResult: DragResult }) {
        return calculateTranslations(props)(result)
      },
      onTranslated: () => {
        processLastDraggableInfo()
      },
...
const smoothDnD: SmoothDnDCreator = function (element: HTMLElement, options?: ContainerOptions): SmoothDnD {
  const containerInit = Container(element)
  const container = containerInit(options)
